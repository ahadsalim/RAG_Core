...
*   **تعریف:** کاربر فایلی فرستاده اما متن او **کاملاً بی‌معنی** است (حروف تصادفی، ناسزا).
*   **⚠️ هشدار بحرانی:** این عبارات **هرگز invalid نیستند** و باید به `general` بروند:
    - "این فایل چیه"، "این چیه"، "چیه این"، "این فایل چیست"
    - "توضیح بده"، "خلاصه کن"، "بررسی کن"، "تحلیل کن"
    - "محتوای فایل چیه"، "درباره فایل بگو"
    - هر سوال یا درخواست معنادار درباره فایل
*   **فقط** وقتی متن واقعاً بی‌معنی است (مثل "اسیبلا" یا "!!!" یا "asdfgh") این دسته را انتخاب کن.
*   **قانون:** اگر شک داری، `general` را انتخاب کن نه `invalid_with_file`.
...
*   **شامل با فایل:** 
    - درخواست‌های ساده درباره فایل: "این فایل چیه"، "توضیح بده"، "خلاصه کن"، "محتوا چیه"
    - فایل‌های آموزشی: جزوه، کتاب درسی، مقاله علمی، تست، آزمون
    - فایل‌های غیرتخصصی: رمان، شعر، مقاله عمومی، عکس
*   **⚠️ قانون مهم:** اگر کاربر فقط می‌خواهد بداند فایل چیست یا محتوایش چیست → `general` (نه business)
...
*   **تعریف:** درخواست همراه با فایل که **محتوای فایل** مرتبط با کسب‌وکار، حقوق، مالیات، بیمه یا امور اداری است.
*   **مثال:** فایل قرارداد + "بررسی کن"، فایل صورتحساب + "توضیح بده"، فایل قانون + "خلاصه کن"
*   **⚠️ این دسته فقط برای فایل‌های تخصصی است:**
    - قرارداد، صورتحساب، اظهارنامه مالیاتی، بیمه‌نامه، سند حقوقی
*   **این فایل‌ها به `general` بروند نه اینجا:**
    - جزوه درسی، کتاب، مقاله علمی، تست آزمون، عکس، رمان، شعر
*   **قانون:** اگر کاربر فقط می‌پرسد "این فایل چیه" → `general` (حتی اگر فایل تخصصی باشد)
