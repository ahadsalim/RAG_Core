...
class OpenAIProvider(BaseLLM):
    ...
    async def generate_with_images(
        ...
        **kwargs
        ...
        """
        Generate a response with image inputs using Vision API.
        
        Args:
            messages: List of text messages
            images: List of image dicts with 'data' (bytes) and 'filename' (str)
            **kwargs: Additional parameters
        ...
        try:
            ...
            for img in images:
                if img.get('data'):
                    ...
                    # Detect image type from filename or default to jpeg
                    filename = img.get('filename', 'image.jpg').lower()
            ...
            logger.info(
                "Generated response with images",
                model=self.config.model,
            ...
            return LLMResponse(
                ...
                metadata={
                    "id": response.id,
                    "created": response.created,
                    "api_type": "chat_completions_vision",
                    "image_count": len(images),
                }
            )
